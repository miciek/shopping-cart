...
class AddToCartSuite extends CatsEffectSuite {
  ...
  test("add a single product to an empty cart") {
    for {
      ...
      newCart <- subject.add(cartRef, "toothpaste", 7)
  ...
  test("fail if product price cannot be fetched") {
    (for {
      cartRef <- Ref[IO].of(Cart.empty)
      _       <- subject.add(cartRef, "non existing", 7)
  ...
  test("add multiple products to an existing cart") {
    for {
      ...
      _       <- List.fill(100)(subject.add(cartRef, "toothpaste", 7)).parSequence
      _       <- List.fill(10)(subject.add(cartRef, "shampoo", 3)).parSequence
      newCart <- cartRef.get
    } yield assertEquals(
      newCart.items.toSet,
      Set(
